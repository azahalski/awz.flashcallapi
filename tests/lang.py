...
module_path = os.path.abspath('../bitrix/modules/awz.flashcallapi/')
lang_prefix = 'AWZ_FLASHCALLAPI_'
...
disabled_lang = (
    ...
    'ACCESS_DENIED',
    'self',
    'AWZ_FLASHCALLAPI_TRANSPORTS_SMSRU_ERR_',
    'AWZ_FLASHCALLAPI_TRANSPORTS_SMSC_ERR_'
)
...
for _ in all_files:
    ...
    if len(no_usage):
        for ln in no_usage:
            check_unusage = False
            for ln_d in disabled_lang:
                if ln.find(ln_d) != -1:
                    check_unusage = True
            if not check_unusage:
